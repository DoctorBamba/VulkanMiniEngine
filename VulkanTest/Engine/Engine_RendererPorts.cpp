...
Engine::RendererWindowPort::RendererWindowPort(CWindow* window_) : p_WindowContext(window_)
{
	...
	const CRenderPass* render_pass = Renderer::RenderPass::output;
...
Void Engine::RendererWindowPort::Display()
{
	...
	TickGraphicsInfo& frame_info = m_FramesInfo.at(buffer_index);
	...
	Engine::Draw_Event(frame_info.draw_task, frame_info.target);
