...
template<class T> class CDX_UAccessVector : public CDX_UAccessObject
{
	protected :
	...
	public:
		...
		CDX_UAccessVector(UINT32 max_num_of_elements, UINT32 flags) : CDX_UAccessObject(max_num_of_elements, sizeof(T), flags) {}
...
template<class T> HRESULT CDX_UAccessVector<T>::push_back(const T &new_element)
{
	HRESULT result = 0;
	...
	if ((BufferFlags & RO_CPU_CAN_WRITE_BUFFER) && !(BufferFlags & RO_USAGE_DYNAMIC_BUFFER))
	{
		...
		result = DeviceContext->Map(Buffer, 0, D3D11_MAP_WRITE, 0, &Map);
		if (FAILED(result))
		{
			return result;
		}
...
template<class T> HRESULT CDX_UAccessVector<T>::set(std::vector<T> *data)
{
	HRESULT result = S_OK;
	...
	if (BufferFlags & RO_CPU_CAN_WRITE_BUFFER)
	{
		...
		result = DeviceContext->Map(Buffer, 0, mapFlags, 0, &Map);
		if (FAILED(result))
		{
...
template<class T> HRESULT CDX_UAccessVector<T>::copyBytes(UINT32 offset, Pointer data_pointer, SIZE_T size_in_element)
{
	HRESULT result = S_OK;
...
template<class T> HRESULT CDX_UAccessVector<T>::at_W(UINT32 index, T data)
{
	HRESULT result = 0;
...
template<class T> const T &CDX_UAccessVector<T>::at_R(UINT32 index)
{
	T out;
	...
	if (BufferFlags & CPU_CAN_READ_RESOURCE)
	{
		...
		HRESULT result = DeviceContext->Map(Buffer, 0, D3D11_MAP_WRITE, 0, &Map);
		if (FAILED(result))
		{
			return result;
		}
