...
{
	public:
		CLocalBuffer(VulkanDevice* device_, Uint allocate_size_, Uint usage_flag_, CGpuUploadTask* upload_task_ = nullptr, Pointer initional_data_ = nullptr)
		...
		Void Upload(CGpuTask* upload_task_, CIntermidiateBuffer* uploader_, Uint offset_ = 0)
		{
			...
			if (uploader_->GetSize() > m_Size - offset_)
			{
			...
			copy_region.dstOffset = offset_;	// Optional
		...
		{
			CIntermidiateBuffer* uploader = new CIntermidiateBuffer(p_DeviceContext, data_size_, CIntermidiateBuffer::Usage::Upload);
		...
		Void Download(CGpuUploadTask* upload_task_, CIntermidiateBuffer* downloader_, Uint offset_ = 0)
		{
			if (!upload_task_->InRecording())
			{
				throw std::runtime_error("CLocalBuffer::Upload Error -> The Upload Task must be in recording!");
			...
			vkCmdCopyBuffer(upload_task_->GetCommandBuffer(), p_Buffer, downloader_->GetVkBuffer(), 1, &copy_region);
		}

		Byte* Download(VkQueue queue_, VkCommandPool command_pool_)
		{
			Byte* data_out = new Byte[m_Size];
			CIntermidiateBuffer* downloader = new CIntermidiateBuffer(p_DeviceContext, m_Size, CIntermidiateBuffer::Usage::Download);

			CGpuDownloadTask* download_task = new CGpuDownloadTask(p_DeviceContext, command_pool_);
			download_task->Reset();
